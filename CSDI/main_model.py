...
class CSDI_base(nn.Module):
    ...
    Time-dependent blending of Gaussian vs. Correlated noise (Chen[2023] schedule).
    ...
    def __init__(self, target_dim, config, device):
        ...
        # 3) 128x128 tile-based correlation => Cholesky
        self.tile_k = 128
        self.tile_l = 128
        self.rho_feat = config["model"].get("rho_feat", 0.5)
        self.rho_time = config["model"].get("rho_time", 0.9)
        self.cov_save_path = config["model"].get("cov_save_path", "cov_matrix_128_128_new.pt")
        ...
        # 5) Standard rectified mapping using Hungarian, only during training
    ...
    # ------------------------------------------------
    # Build or load single (128x128) covariance => store Cholesky
    # ------------------------------------------------
    ...
    # ------------------------------------------------
    # Generate a single tile => shape(B,128,128)
    # ------------------------------------------------
    ...
    # 1D freq-based "blue" (maybe add this)
    def generate_blue_noise_time(self, shape):
        B,K,L = shape
        noise = torch.randn(B,K,L, device=self.device)
        freq_dom = torch.fft.fft(noise, dim=-1)
        freq_filter = torch.linspace(0.0,1.0,L, device=self.device)**2
        freq_filter = freq_filter.view(1,1,L)
        filtered = freq_dom*freq_filter
        mask_blue = torch.fft.ifft(filtered, dim=-1).real
        return mask_blue
